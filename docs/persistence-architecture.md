...
The API now intentionally allows anonymous browser writes, as agreed for this app. The GitHub token remains server-side in the Worker and is never sent to the browser.

Writes use optimistic concurrency: the browser sends the GitHub file SHA it last read, and the Worker rejects a write if another device changed the file first. This prevents one device from silently overwriting another device's newer recipe list.
